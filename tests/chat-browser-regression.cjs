...
(async () => {
  ...
  try {
    for (const [width,height,mobile] of [[375,667,true],[390,844,true],[844,390,true],[430,915,true],[1280,900,false]]) {
      ...
      await page.addScriptTag({content:fs.readFileSync(path.join(repo,'main.js'),'utf8')});
      ...
      const first=await page.evaluate(()=>({empty:!!document.querySelector('.solomon-chat-empty'),top:document.querySelector('.solomon-chat-bubble').getBoundingClientRect().top,viewTop:leaf.view.contentEl.getBoundingClientRect().top}));
      assert.equal(first.empty,false);assert.ok(first.top>=first.viewTop);
      if(width===390)await page.screenshot({path:path.join(repo,'docs/testing/iphone-message-containment-regression.png')});
      await page.evaluate(async()=>{for(let i=0;i<30;i++)append();draw();await settle();});
