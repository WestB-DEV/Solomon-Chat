...
document.querySelector<HTMLElement>("#device-label")!.textContent = android ? "Pixel 9" : "iPhone 15";
let open = query.get("keyboard") === "open";

function render(): void {
  const visibleHeight = open ? deviceHeight - keyboardHeight : deviceHeight;
  const layout = calculateViewportLayout({ mobile: true, focused: open, layoutHeight: deviceHeight, visualHeight: visibleHeight, visualOffsetTop: android ? 24 : 0, containerBottom: deviceHeight, closedToolbarClearance: toolbarHeight });
